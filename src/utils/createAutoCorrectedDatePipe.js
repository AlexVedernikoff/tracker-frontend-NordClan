...
// This is a date pipe creator from text-mask-addons library with a little tweaks
...
export default function createAutoCorrectedDatePipe(dateFormat = 'mm dd yyyy', { minYear = 1, maxYear = 9999 } = {}) {
  ...
  return function(conformedValue) {
    ...
    // Validate completely entered date
    if (conformedValue.replace(/\D/g, '').length === dateFormatArray.join('').length) {
      isInvalid = !moment(conformedValue, dateFormat.toUpperCase()).isValid();
    }
