...
const TestPlansTable: FC<TestPlansTableProps> = (props: TestPlansTableProps) => {
  ...
  const [confirmDeleteComponent, confirmDelete] = useConfirmModal(
    loc.DELETE_SUBMIT_CONFIRM,
    (plan_id?: number) => plan_id && deleteTestPlan(plan_id)
  );
    ...
    _.orderBy(testPlans, (plan: TestsPlan) => plan.createdAt.unix).map(plan => (
      <div className={css.row} key={plan.id} onClick={() => editPlan(plan.id)}>
        ...
        <div className={css.buttons}>
          ...
          >
            <Button type="primary" text="" icon="IconFileExel" name="right" />
          </a>
          <Button
            onClick={e => {
              e.stopPropagation();
  ...
  return (
    <div>
      ...
      {!testPlansLoading && <Pagination itemsCount={pagesCount} activePage={activePage} onItemClick={setPage} />}
