...
import { TASK_CHANGE_REQUEST_SUCCESS } from '../../constants/Task';
...
function onUpdateTracks(state, action) {
  ...
  const updatedTracks = state.tracks[action.timesheet.onDate].tracks.map(track => {
    const taskId = getTaskId(action);
    const isDraft = taskId && track.taskId === taskId && action.timesheet.isDraft;
    return track.id === action.timesheet.id || isDraft ? { ...track, ...action.timesheet } : track;
  });
...
exports[CREATE_TIMESHEET_SUCCESS] = (state = InititalState, action) => {
  ...
  const updatedTracks = [...state.tracks[action.timesheet.onDate].tracks, action.timesheet].filter(track => {
...
exports[DELETE_TIMESHEET_SUCCESS] = (state = InitialState, action) => {
  ...
  const updatedTracks = state.tracks[action.timesheet.onDate].tracks.filter(track => {
...
function updateTracks(state, action, updatedTracks) {
  ...
  const all = updatedDay[action.timesheet.onDate].tracks.reduce(
    (acc, track) => exactMath.add(acc, track.spentTime || 0),
    0
  );
...
exports[TASK_CHANGE_REQUEST_SUCCESS] = (state = InitialState, action) => {
  const updatedState = Object.entries(state.tracks).reduce(
    (acc, [day, { tracks, scales }]) => {
