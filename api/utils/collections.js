 ...
 * @exports conditionalForEach - итератор по массиву,
 * выполняющий некоторое действие с каждым элементом при некотором условии
 */
...
export function findUnique(arr) {
  ...
  const nextArguments = Array.prototype.slice.call(arguments, 0);
...
/**
 * Выполняет произвольный код для каждого элемента массива и дополнительного параметра,
 * если они удовлетворяют некоторому условию
 ...
 * - выражение, которое выполняется при выполнении условия
 * (функция от элемента массива и дополнительного аргумента)
 * @returns {*} - первый аргумент (предположительно, массив, если массивом не является,
...
export function conditionalForEach(arr, second, condition, expression) {
  if (arr instanceof Array) {
    ...
    arr.forEach((first) => {
      !!_condition(first, second) && _expression(first, second); // eslint-disable-line
    });
  }
  return arr;
}
 ...
 * @returns {function} - "чистая" функция (переданная в качестве параметра,
 * или созданная заглушка, всегда возвращающая true)
 */
const proofy = func => (func && typeof func === 'function') ? func : () => true;
